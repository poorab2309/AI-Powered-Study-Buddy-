...
When your answer includes mathematical expressions, formulas, or equations, format them using LaTeX notation with $ for inline math (e.g. $x^2 + y^2$) or $$ for block equations. This ensures they display correctly and don't lose formatting like exponents or fractions.

If the student asks you to solve a problem or work through a calculation, show your reasoning step by step, not just the final answer.
...
def answer_question(question: str, history: list = None, k: int = 4) -> dict:
    ...
    try:
        ...
        if "RESOURCE_EXHAUSTED" in str(e):
            return {
                "answer": "I've hit the daily free-tier limit for AI requests. Please try again later or check your API quota.",
                "sources": [],
    ...
    if answer_text.startswith(WEB_SEARCH_MARKER) and tavily_client:
        ...
        try:
            ...
            if "RESOURCE_EXHAUSTED" in str(e):
                answer_text = "I found relevant info but hit the daily API limit trying to combine it with a web search. Please try again later."
            else:
                answer_text = (
                    f"I wanted to search the web for this but hit an error: {e}"
                )
